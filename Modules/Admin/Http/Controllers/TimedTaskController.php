...
class TimedTaskController extends CrudController {
    ...
    public function store(Request $request) {
        try {
            ...
            $input['command'] = $this->MakeCommand(
                $task_id, $log_path, $input['time_type'], $input['day'], $input['hour'], $input['minute'],
                $input['week_day']
            );
                        ...
                        foreach ($input['site_id'] as $key => $value) {
                            ...
                            $data = [
                                ...
                                'command'    => $this->MakeCommand(
                                    $childTaskId, $childLogPath, $input['time_type'], $input['day'], $input['hour'],
                                    $input['minute'], $input['week_day']
                                ),
                            ];
    ...
    public function ExecuteTask(Request $request) {
        try {
            ...
            if ($task->category == 'index' && $task->parent_id == '0') {
                $childrenTaskIds = $this->ModelInstance()->where('parent_id', $id)->where('status', 1)->pluck('id')
                                        ->toArray();
    ...
    public function TimedTaskQueue($ids, $action) {
        try {
            foreach ($ids as $id) {
                $dataArr = [
                    'class'  => TimedTaskController::class,
                    'method' => 'DoTimedTask',
            ...
            switch ($doAction) {
                case 'add':
                    if (!in_array($task->command, $CrontabList)) {
                        ...
                        // 设置文件权限
                        file_put_contents(
                            $this->ErrorLog, "\r"."chmod 700 ".$this->TaskPath.$task->task_id, FILE_APPEND
                        );
