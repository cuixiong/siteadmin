...
//            $perms = array_merge($perms,$perms2);
            ...
            $data['roles'] = $res['code'];
            $rule_ids = $res['rule'];
            $perms = $RuleModel->where('type', 'BUTTON')->whereIn('id', $rule_ids)->where($where)->where('status', 1)
                               ->select([$NameFiled, 'perm'])->get('perm')->toArray();
            if ($siteId > 0) {
                // Why do you do this? Ask your leader the reason
//            $perms2 = $RuleModel->where('type','BUTTON')->where('parent_id','172')->where('status',1)->select([$NameFiled,'perm'])->get()->toArray();
//            $perms = array_merge($perms,$perms2);
    ...
    public function menus(Request $request) {
        ...
        // 查询type=1的菜单类型的权限信息
        $model = new Rule();
        $fields = ['id', 'parent_id', 'path', 'component', 'redirect', 'perm', 'icon', 'type', 'sort', 'category',
                   'visible', 'keepAlive', 'updated_by', 'updated_at', 'created_at', 'created_by'];
        ...
        if ($is_super > 0) {
            $rules = $model->select($fields)->whereIn('type', ['CATALOG', 'MENU', 'EXTLINK'])->where($where)->orderBy(
                'sort', 'asc'
            )->orderBy('created_at', 'desc')->get()->toArray();
        } else {
            $rules = $model->select($fields)->whereIn('id', $rule_ids)->whereIn('type', ['CATALOG', 'MENU', 'EXTLINK'])
                           ->where($where)->orderBy('sort', 'asc')->orderBy('created_at', 'desc')->get()->toArray();
        }
        // 递归分类权限
