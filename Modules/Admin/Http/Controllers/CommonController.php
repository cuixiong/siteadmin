...
class CommonController extends Controller
{
    ...
    public function menus(Request $request){
        ...
        $where = $siteId > 0 ? ['category' => 2,'status' => 1] : ['category' => 1,'status' => 1];
        if($is_super > 0){
            $rules = $model->select($fields)->whereIn('type',['CATALOG','MENU'])->where($where)->orderBy('sort','asc')->orderBy('created_at','desc')->get()->toArray();
        } else {
            $rules = $model->select($fields)->whereIn('id',$rule_ids)->whereIn('type',['CATALOG','MENU'])->where($where)->orderBy('sort','asc')->orderBy('created_at','desc')->get()->toArray();
        }
        // 递归分类权限
