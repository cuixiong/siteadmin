...
class PriceEdition extends Base {
    ...
    public static function SaveToSite($type = self::SAVE_TYPE_FULL, $id = null, $isAllSite = false, $siteIds = null) {
        ...
        $priceEditionValueData = DB::table($priceEditionValueTableName)->orderBy('sort', 'asc')->orderBy('id', 'asc')
                                   ->get()->toArray();
        // 整理每个出版商对应的价格版本数组 开始
        ...
        foreach ($priceEditionData as $key => $edition) {
            ...
            if (count($publishers) > 0) {
                foreach ($publishers as $key => $publisher) {
                    ...
                    $publisherPriceEditionData[$publisher] = array_merge(
                        $publisherPriceEditionData[$publisher], $priceEditionValueByEditionId[$edition['id']]
                    );
                }
            } else {
                continue;
            ...
            if ($type == self::SAVE_TYPE_FULL) {
                ...
                foreach ($priceEditionData as $record) {
                    ...
                    if ($commonPublisher && count($commonPublisher) > 0) {
                        ...
                        $priceEditionDataSql[] = "INSERT INTO $priceEditionTableName (".implode(", ", $columns)
...
//                    }
                    ...
                    $priceEditionValueDataSql[] = "INSERT INTO $priceEditionValueTableName (".implode(", ", $columns)
                                                  .") VALUES (".implode(", ", $values).");";
                }
                // 复制数据到分站点数据库
