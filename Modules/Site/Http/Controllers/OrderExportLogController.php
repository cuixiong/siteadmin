...
class OrderExportLogController extends CrudController {
    /**
     * 导出进度
     *
     * @param $request 请求信息
    ...
    public function exportProcess(Request $request) {
        ...
        $logData = OrderExportLog::where('id', $logId)->first();
        if ($logData) {
        ...
        $text = '';
        $updateTime = 0;
        if ($logData['state'] != OrderExportLog::UPLOAD_COMPLETE) {
            $data['result'] = false;
        }
    ...
    public function exportFileDownload(Request $request) {
        ...
        if ($logData['state'] == OrderExportLog::EXPORT_COMPLETE) {
