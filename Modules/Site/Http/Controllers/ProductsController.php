            ...
            $hasFieldArray = array_unique(
                array_merge(
                    ...
                    ['classification', 'application', 'updated_at', 'updated_by', 'created_at',
                     ...
                     'discount_time_begin', 'discount_time_end', 'price_values']
                )
            );
...
//                $index->flushIndex();
    ...
    public function destroy(Request $request)
    {
        try {
            ...
            if ($type == 1) {
                $subjectCount =  PostSubject::query()->whereIn('product_id', $ids)->where('propagate_status', 0)->count() ?? 0;
    ...
    function batchDelete(
        ...
        if ($type == 1) {
            // 总数量
            $data['count'] = $model->count();
            $data['subject_count'] =  PostSubject::query()->whereIn('product_id', $ids)->where('propagate_status', 0)->count() ?? 0;
            ReturnJson(true, trans('lang.request_success'), $data);
        } elseif ($type == 2) {
            $subjectLogDetail = '';
            $deleteProductIds = [];
