...
class AliyuncsOss {
    ...
    /**
     * 移动文件
     * @param $oldFile     string linux文件路径
     * @param $newOssPath  string oss新目标文件路径
     * @param $oldOssPath  string oss旧目标文件路径
     * @param $bucket
     *
     * @return string|true
     */
    public function move($oldFile, $newOssPath, $oldOssPath, $bucket = '') {
        try {
            $this->setBucket($bucket);
            //暂时注释
            //$this->ossClient->copyObject($this->bucket, ltrim($oldFile,'/'), $this->bucket, ltrim($newFile,'/'));
            $file = ltrim($newOssPath, '/');
            $this->ossClient->uploadFile($this->bucket, $file, $oldFile);
            if ($newOssPath != $oldOssPath) {
                $this->ossClient->deleteObject($this->bucket, ltrim($oldOssPath, '/'));
            }
